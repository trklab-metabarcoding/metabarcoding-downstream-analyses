...
params:
  ...
  phyloseq_name: "test_diet_global_20240502.rds" # ex.,"test_diet_local_global_20250408.rds"
...
if (params$phyloseq_from_4a == "Y") {
  # RDS generated in Step4a (global only)
  phyloseq_object <- file.path(params$data_path, params$project_code, "merged_runs", params$step_3a_analysis_folder, params$phyloseq_name)
} else if (params$phyloseq_from_4b == "Y") {
  # RDS generated in Step4b (global and local)
  phyloseq_object <- file.path(params$data_path, params$project_code, "merged_runs", params$step_3a_analysis_folder, params$phyloseq_name)
} else if (params$phyloseq_from_manual == "Y") {
  # Manually copied RDS
  phyloseq_object <- params$phyloseq_name
} else {
  stop("One of the phyloseq parameters needs to be set to 'Y'")
}
```

Run this block for global and local - RDS generated in Step4b
```{r}
phyloseq_object <- file.path(params$data_path, params$project_code, "merged_runs", params$step_3a_analysis_folder, params$phyloseq_name)
```

Run this block if you manually copied your RDS
```{r}
phyloseq_object <- params$phyloseq_name
```
...
saveRDS(physeq_rarefy_growth_forms, final_filename)
```
